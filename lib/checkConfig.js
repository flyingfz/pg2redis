    ...
    for (const tableName of tableNameList) {
        ...
        for (const config of subConfig) {
            ...
            if(typeof config.expired != "number"){
                errMsg.push(`${tableName} 表的 前缀为: ${config.key_prefix} 的配置里， expired 的值 : [${config.expired}] 不是数字. `)
            }else if(config.expired < 0 ){
                errMsg.push(`${tableName} 表的 前缀为:${config.key_prefix} 的配置里， expired 的值 : [${config.expired}] 小于0. `)
            }else if( String(config.expired).indexOf(".") > -1){
                errMsg.push(`${tableName} 表的 前缀为:${config.key_prefix} 的配置里， expired 的值 : [${config.expired}] 不是整数. `)
            }
            ...
            if(config.value_field.length > 1 && config.value_type != "hash"){
                ...
                errMsg.push(`${tableName} 表的 前缀为:${config.key_prefix} 的配置里， value_field 个数超过1个 ， 但 value_type 不是 hash .`);
            }else if(config.value_field.length == 1 && ![ "hash", "set" , "string" ].includes(config.value_type)){
                errMsg.push(`${tableName} 表的 前缀为:${config.key_prefix} 的配置里， value_field 个数是1个 ， 但 value_type 不是 hash 、set 、string .`);
            }
        }
